...
class WhiterabbitControllerConfig(DirectionalTradingControllerConfigBase):
    controller_name = "whiterabbit"
    ...
    rsi_length: int = Field(
        default=21,
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the RSI lenght: ",
            prompt_on_new=True))
...
class WhiterabbitController(DirectionalTradingControllerBase):
    ...
    def __init__(self, config: WhiterabbitControllerConfig, *args, **kwargs):
        self.config = config
        self.max_records = max(config.rsi_length, config.bb_length)
    ...
    def get_processed_data(self) -> pd.DataFrame:
        ...
        # Combine signals
        long_condition = (bbp < self.config.bb_long_threshold) & (rsi < 10)
        short_condition = (bbp > self.config.bb_short_threshold) & (rsi > 90)
        df["signal"] = 0
